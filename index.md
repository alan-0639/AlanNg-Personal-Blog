---
# https://vitepress.dev/reference/default-theme-home-page
# http://localhost:5173/AlanNg-Personal-Blog/
layout: home


hero:
  name: "Alan Ng 個人博客"
  text: ""
  tagline: 客制化機械鍵盤作品
  image:
   src: /個人卡通頭像.png
   alt: Alan Ng

  actions:
    - theme: brand
      text: 創作歷程
      link: '/zh-HK/HAWKZ-dim-sum-102/PCB設計'

    - theme: brand
      text: 客制化之旅
      link: '/zh-HK/基础定制/键盘内胆/'




features:
  - title: •	硬體設計
    details: PCB 原理圖、雙層 PCB Layout Design、PCB 打樣、3D 建模
    link: 
    target: blank

  - title: •	軟/硬體工具
    details: 嘉立創 EDA、Fusion360、AutoCAD、VS Code、電烙鐵、風槍、加熱台、J-link、ST-link、萬用表
    link: 
    target: blank

  - title: •	系統操作
    details: ERP（E10、企雲樂 等）、BOM 管理、工程變更 (ECO)
    link: 
    target: blank

  - title: •	專案協調
    details: 工程打樣與工廠對接、產品測試與除錯、固件版本管控、技術規格書撰寫
    link: 
    target: blank

  - title: 預設-支援Vial即時改鍵
    details: 真正的即時生效，無需編譯，無需刷寫韌體。靈感與設定同步，想到即改，改完即用。
    link: 
    target: blank

  - title: 預設-支援三模連接
    details: 支援數據綫模式  <br>支援藍牙1/藍牙2/藍牙3模式  <br>支援接收器模式
    link: 
    target: blank

  - title: 基礎-姓名/LOGO定制
    details: 這不是一把量產鍵盤，是你的領地。<br>刻上你的名，印上你的徽，從此，每一次敲擊都是自我的迴響。
    link: 
    target: blank

  - title: 基礎-常規軸和矮軸
    details: 探索高軸系列-傳統機械的飽滿旅程  <br>發現矮軸世界-極致觸發的迅捷美學
    link: 
    target: blank

  - title: 基礎-基礎聲音套件
    details: 夾心棉、軸下墊、底棉等填充的護航，告別空腔雜音，抵達純淨之境。
    link: 
    target: blank    

  - title: 基礎-自定義按鍵佈局
    details: 單獨的PCB原理圖和電路圖的設計。 <br>終極的客製化，從重塑佈局開始。
    link: 
    target: blank

  - title: 高級-重塑鍵盤結構
    details: 根據需求設計GASKET、TOP、BOTTOM、船、三明治、無鋼等結構
    link: 
    target: blank

  - title: 高級-外殼3D圖設計
    details: 根據需求重新繪製鍵盤外殼和內部細節，與PCB、聲音包等尺寸相互兼容
    link: 
    target: blank



---

###

# 機械鍵盤愛好者的樂園

  暫不支持OLED和ENCODER，敬請期待......
